...
def clear_cart():
    ...
    confirm = messagebox.askyesno("ინფორმაცია", "დარწმუნებული ხარ რომ გსურს კალათის გასუფთავება?")
    if confirm:
        cart.clear()
        update_cart_display()
...
def complete_sale():
    ...
    try:
        ...
        conn.commit()
        messagebox.showinfo("ინფორმაცია", "გაიყიდა წარმატებით!")
...
def add_product():
    ...
    messagebox.showinfo("ინფორმაცია", "პროდუქტი დაემატა წარმატებით!")
    show_products()
...
def delete_product():
    ...
    confirm = messagebox.askyesno("ინფორმაცია", f"დარწმუნებული ხარ რომ გსურს '{product_name}'-ის წაშლა?")
    if confirm:
        conn = connect_db()
        cur = conn.cursor()
        ...
        messagebox.showinfo("ინფორმაცია", "პროდუქტი წაიშალა")
        show_products()
...
def edit_product():
    ...
    def save_changes():
        ...
        try:
            ...
            messagebox.showinfo("ინფორმაცია", "პროდუქტი განახლდა")
            edit_window.destroy()
            show_products()
        except ValueError:
...
lbl_cart_total = ttk.Label(total_frame, text="0.00 ₾", font=("Segoe UI", 24, "bold"), foreground="#27ae60")
...
ttk.Button(total_frame, text="კალათიდან ამოღება", command=remove_from_cart, style="Delete.TButton")\
    .pack(pady=5, padx=20, fill="x")
...
cart_tree = ttk.Treeview(cart_frame, columns=("N", "დასახელება", "რაოდ.", "ფასი", "ჯამი"), show="headings", height=12)
